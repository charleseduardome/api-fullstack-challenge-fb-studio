...
let availabilityQuestion: AvailabilityQuestionService;

describe('AvailabilityQuestion', () => {
  beforeEach(() => {
    fakeQuestionsRepository = new FakeQuestionsRepository();
    createQuestion = new CreateQuestionService(fakeQuestionsRepository);
    availabilityQuestion = new AvailabilityQuestionService(
      fakeQuestionsRepository,
    );
  });

  it('shoul be able to update a availability question', async () => {
    ...
    const updtQuestion = await availabilityQuestion.execute({
      id: String(question.id),
      disponivel: false,
    });

    expect(updtQuestion.disponivel).toBe(false);
  });

  it('shoul not be able to update the question non-existing question', async () => {
    await expect(
      availabilityQuestion.execute({
